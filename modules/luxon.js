...
class Formatter {
  ...
  formatDateTimeFromString(dt, fmt) {
    ...
    const meridiem = () => (knownEnglish ? meridiemForDateTime(dt) : string({
      hour: 'numeric',
      hourCycle: 'h12',
    }, 'dayperiod'));
    const month = (length, standalone) => (knownEnglish ? monthForDateTime(dt, length) : string(standalone ? {
      month: length,
    } : {
      month: length,
      day: 'numeric',
    }, 'month'));
    const weekday = (length, standalone) => (knownEnglish ? weekdayForDateTime(dt, length) : string(standalone ? {
      weekday: length,
    } : {
      weekday: length,
      month: 'long',
    ...
    const era = (length) => (knownEnglish ? eraForDateTime(dt, length) : string({
      era: length,
    }, 'era'));
